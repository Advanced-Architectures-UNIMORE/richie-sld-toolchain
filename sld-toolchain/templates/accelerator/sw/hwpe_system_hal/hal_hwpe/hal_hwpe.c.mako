...
#ifndef __HAL_HWPE_CL${cluster_id}_LIC${accelerator_id}_H__
#define __HAL_HWPE_CL${cluster_id}_LIC${accelerator_id}_H__
...
static inline void hwpe_packet_size_set_${acc_wr_stream_in[i]}(unsigned int value) {
  HWPE_WRITE(value, ${acc_wr_target.upper()}_REG_PACKET_SIZE_${acc_wr_stream_in[i].upper()});
}
  % endfor
% endif
...
static inline int hwpe_get_finished() {
  return HWPE_READ(${acc_wr_target.upper()}_REG_FINISHED);
}
