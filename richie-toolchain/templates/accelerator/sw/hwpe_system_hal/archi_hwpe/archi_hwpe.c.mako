...
<%
    if (extra_param_0 == None):
        cluster_id = 0
    else:
        cluster_id = int(extra_param_0)

    if (extra_param_1 == None):
        accelerator_id = 0
    else:
        accelerator_id = int(extra_param_1)
%>
\
/*
...
#define __ARCHI_HWPE_CL${cluster_id}_LIC${accelerator_id}_H__
...
<%
    # Calculate peripheral base address for HWPE accelerator.
    # This is calculated using an accelerator ID that is passed
    # through 'extra_param_1'.

    # The Richie Cluster reserves HWPE peripheral addresses starting from 0x1b202000
    # with a fixed size of 0x200 for each accelerator.

    speriph_hwpe_addr = int(0x1b202000) + int(0x200) * accelerator_id
%>

/* HWPE base address (with cluster alias) */

#define ARCHI_HWPE_ADDR_BASE ${ hex(speriph_hwpe_addr) }
