...
static inline void hwpe_packet_size_set_${stream_in[i]}(unsigned int value) {
  HWPE_WRITE(value, ${target.upper()}_REG_PACKET_SIZE_${stream_in[i].upper()});
}
  % endfor
% endif
...
static inline int hwpe_get_finished() {
  return HWPE_READ(${target.upper()}_REG_FINISHED);
}
