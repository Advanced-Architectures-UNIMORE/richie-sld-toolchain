...
'''
    ...
    Description:    Accelerator-rich HeSoC testbench.
...
module richie_tb #(
  ...
  parameter int unsigned  N_CORE_CL_${i}        = pulp_cluster_${i}_cfg_pkg::N_CORES,
  parameter int unsigned  N_TCDM_BANKS_CL_${i}  = pulp_cluster_${i}_cfg_pkg::N_TCDM_BANKS,
  parameter bit           L1_AMO_PRESENT_${i}   = pulp_cluster_${i}_cfg_pkg::L1_AMO_PRESENT,
  % endfor

  // HeSoC Parameters
  parameter int unsigned  N_CLUSTERS            = hesoc_cfg_pkg::N_CLUSTERS,
  parameter int unsigned  AXI_DW                = 128,

  // L2 Parameters
  parameter int unsigned  L2_N_AXI_PORTS        = hesoc_cfg_pkg::L2_N_AXI_PORTS,
  parameter bit           L2_AMO_PRESENT        = hesoc_cfg_pkg::L2_AMO_PRESENT
);
  ...
  // Fill TCDM memory.
  % for i in range(n_clusters):
  for (genvar iBank = 0; iBank < N_TCDM_BANKS_CL_${i}; iBank++) begin: gen_fill_tcdm_bank_${i}
    initial begin
      $display("Reading SLM file: %s/slm_files/l1_0_%01d.slm", getenv("VSIM_PRJ_PATH"), iBank);
      $readmemh($sformatf("%s/slm_files/l1_0_%01d.slm", getenv("VSIM_PRJ_PATH"), iBank),
        dut.gen_clusters[${i}].gen_cluster_sync.i_cluster.i_ooc.i_bound.gen_tcdm_banks[iBank].i_tc_sram.sram);
    end
  end
  % endfor
    ...
    for (genvar iPort = 0; iPort < L2_N_AXI_PORTS; iPort++) begin: gen_fill_l2_ports
      for (genvar iRow = 0; iRow < N_SER_CUTS; iRow++) begin: gen_fill_l2_rows
        for (genvar iCol = 0; iCol < N_PAR_CUTS; iCol++) begin: gen_fill_l2_cols
          ...
          initial begin
            $display("Reading SLM file: %s/slm_files/l2_%01d_%01d.slm", getenv("VSIM_PRJ_PATH"), file_ser_idx, iCol);
            $readmemh($sformatf("%s/slm_files/l2_%01d_%01d.slm", getenv("VSIM_PRJ_PATH"), file_ser_idx, iCol),
  ...
  // Observe HeSoC interconnect for errors.
