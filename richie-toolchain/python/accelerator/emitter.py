...
class AcceleratorEmitter:
    ...
    def __init__(self, accelerator_design_knobs, dir_out_hwpe):
        ...
        self.out_hwpe                                   = os.path.join(dir_out_hwpe, accelerator_design_knobs.target)

        # Hardware
        self.out_gen_wrap                               = self.out_hwpe + '/wrap'
        self.out_gen_rtl                                = self.out_hwpe + '/rtl'
        self.out_gen_acc_kernel                         = self.out_hwpe + '/rtl/acc_kernel'

        # Test
        self.out_gen_test                               = self.out_hwpe + '/test'

        # Standalone test - Hardware
        self.out_gen_standalone_test_hw                 = self.out_gen_test + '/hw'

        # Standalone test - Software
        self.out_gen_standalone_test_sw                 = self.out_gen_test + '/sw'
        self.out_gen_standalone_test_hwpe_lib           = self.out_gen_test + '/sw/inc/hwpe_lib'

        # System test - Software
        self.out_gen_system_test_hwpe_lib               = self.out_hwpe + '/../../test/sw/inc/wrappers/' + accelerator_design_knobs.target + '/hwpe_lib'
    ...
    device types (HWPE, testbench, sw, etc.) with a particular file_name constructor;
    ...
    the generated component is devoted to (HWPE? Testbench? Others?). Each device
    ...
    '''
    Defines a dictionary that associates the supported device types (HWPE, testbench, sw, etc.)
    ...
    def get_dict_file_ext(self):
        ...
        dict_file_ext = {
            'hw'                : { "sv": ".sv" } ,
            'integr_support'    : { "yml": ".yml", "lock": ".lock", "vsim_wave": ".tcl" } ,
